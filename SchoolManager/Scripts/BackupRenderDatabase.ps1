...
# Render: RENDER_PG_HOST, RENDER_PG_DATABASE, PGPASSWORD; opcional RENDER_PG_USER, RENDER_PG_PORT
$renderHost = $env:RENDER_PG_HOST
$renderPort = if ($env:RENDER_PG_PORT) { $env:RENDER_PG_PORT } else { "5432" }
$renderDatabase = $env:RENDER_PG_DATABASE
$renderUsername = if ($env:RENDER_PG_USER) { $env:RENDER_PG_USER } else { "admin" }
$renderPassword = $env:PGPASSWORD
if ([string]::IsNullOrWhiteSpace($renderHost) -or [string]::IsNullOrWhiteSpace($renderDatabase) -or [string]::IsNullOrWhiteSpace($renderPassword)) {
    Write-Host "Defina RENDER_PG_HOST, RENDER_PG_DATABASE y PGPASSWORD." -ForegroundColor Red
    exit 1
}
